...
// jumlah partikel — cukup padat buat siluet kebaca, masih enteng buat loop JS per frame
const COUNT = 6500
...
export function ParticleFace({ position = [0, -36.55, 1.5] }) {
  ...
  useFrame((state, delta) => {
    ...
    for (let i = 0; i < COUNT; i++) {
      ...
      arr[i3] += (tp[i3] + wx - arr[i3]) * k
      arr[i3 + 1] += (tp[i3 + 1] + wy - arr[i3 + 1]) * k
      arr[i3 + 2] += (tp[i3 + 2] - arr[i3 + 2]) * k
  ...
  return (
    <group ref={group} position={position} visible={false}>
      <points ref={points}>
        ...
        <pointsMaterial ref={mat} vertexColors size={0.05} sizeAttenuation transparent opacity={0} depthWrite={false} />
      </points>
    </group>
  )
}
